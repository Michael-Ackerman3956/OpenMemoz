...
export default function EditionPage() {
  ...
  return (
    <>
      ...
      {/* Masthead bar */}
      <div className="border-b border-rule bg-surface px-5 py-2 md:px-7">
        <div className="mx-auto flex max-w-[1120px] items-center justify-between text-[9px] uppercase tracking-[0.12em] text-muted">
          <span>{formatEditionDate(edition.editionDate)}</span>
          <span>
            Edition No. {edition.editionNumber} &middot;{" "}
            {filteredStories.length} of {edition.storyCount} stories
          </span>
        </div>
      </div>
      ...
      <main className="mx-auto max-w-[1120px] px-5">
        {layout.heroStory ? (
          <>
            <HeroStory story={layout.heroStory} onSelectStory={selectStory} />
            ...
            {layout.mode === "dynamic" && (
              <>
                ...
                <div className="flex items-center justify-between bg-[#0A0908] px-4 py-1.5 text-[9px] font-bold uppercase tracking-[0.12em] text-ink">
                  <span>
                    Today&rsquo;s Edition &middot; {filteredStories.length}{" "}
                    Stories
                  </span>
      ...
      <footer className="mt-px border-t-[3px] border-double border-rule">
        <div className="mx-auto max-w-[1120px] space-y-2 px-5 py-10 font-sans text-xs leading-relaxed text-muted">
          <p>
            ...
            &mdash; an agent-readable newspaper. This page exposes six WebMCP
            tools via <code>document.modelContext</code>.
          </p>
          <p>
            Sources: Hacker News &middot; Federal Reserve &middot; NOAA &middot;
            EurekAlert &middot; ScienceDaily &middot; NVD &middot; TechCrunch
            &middot; NASA &middot; SEC EDGAR
          </p>
          <p>
            Built for the WebMCP Challenge &middot; Apache-2.0 &middot; &copy;
            2026 Nestuary Wellness Inc.
          </p>
        </div>
      </footer>
